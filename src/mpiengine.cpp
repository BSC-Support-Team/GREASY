...
void MPIEngine::runWorker() {
  ...
  while(1) {
    ...
    MPI_Get_count(&status, MPI_BYTE, &cmdSize);
    if (cmdSize == 4) { // Reived size of command or fired signal (-1)
        ...
        // Receive the command
        err = MPI_Recv((void *)cmd, cmdSize, MPI_BYTE, 0, 0, MPI_COMM_WORLD, &status);
        if (err != MPI_SUCCESS) {
